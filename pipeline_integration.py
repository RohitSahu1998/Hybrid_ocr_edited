...
def match_qwen_to_ocr(qwen_json, ocr_results):
    ...
    for key, qwen_value in flattened_qwen.items():
        ...
        for idx, ocr_box in enumerate(ocr_results):
            ...
            if idx not in used_ocr_indices and box_text_clean in target_clean:
                matched_data.append({
                    "Key": key,
            ...
            # Case 2: The Qwen field is a smaller piece inside a huge OCR line (e.g. Qwen: "301", OCR: "Sally Yessler 301 W Spring St")
            # We must calculate a mathematical sub-bounding-box and we DO NOT consume the physical box!
            elif target_clean in box_text_clean and len(target_clean) >= 4:
                # Find character indices
                start_idx = box_text_clean.find(target_clean)
                ...
                # Notice we intentionally omit `used_ocr_indices.add(idx)` here, so other keys can extract from the remaining part!
...
if __name__ == "__main__":
    ...
    # highlight_matches_on_image("Document_1.jpg", matched_results, "output_highlighted.jpg")
